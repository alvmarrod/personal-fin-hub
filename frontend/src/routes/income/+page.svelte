<script>
  ...
  import { formatDate as formatDateLocale, formatAmount, maskAmount } from '$lib/utils/format.svelte';
          ...
          <tbody>
            {#each incomeSources as source (source.name + source.currency + source.category)}
              <tr>
                ...
                <td class="num">{maskAmount(formatAmount(source.realized, source.currency))} {source.currency}</td>
                <td class="num">{maskAmount(formatAmount(source.projected, source.currency))} {source.currency}</td>
                <td class="num">{maskAmount(formatAmount(source.total, source.currency))} {source.currency}</td>
                <td>
                  {#if source.schedule}
                ...
                <td>
                  {#if source.schedule}
                    <button class="icon-btn" title="Edit schedule" aria-label="Edit schedule" onclick={() => editSchedule = source.schedule}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    <button class="icon-btn icon-btn-danger" title="Delete schedule" aria-label="Delete schedule" onclick={() => deleteSchedule = source.schedule}>
          ...
          <tbody>
            {#each paginatedRecentIncome as tx (tx.id)}
              <tr>
                ...
                <td class="num">{maskAmount(formatAmount(parseNum(tx.total_value), tx.currency))}</td>
                <td>{tx.currency || '-'}</td>
