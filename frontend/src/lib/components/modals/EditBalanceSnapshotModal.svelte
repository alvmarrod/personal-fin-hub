<script>
  ...
  let { open, snapshot, onclose, onsuccess } = $props();
  ...
  function populateForm() {
    if (!snapshot) return;
    entityId = String(snapshot.entity_id);
    currency = snapshot.currency;
    amount = String(snapshot.amount);
    timestamp = new Date(snapshot.timestamp).toISOString().slice(0, 16);
  ...
  async function handleSubmit() {
    ...
    try {
      await crud.balanceSnapshots.update(snapshot.id, {
      ...
      error = e.message || t('modals.updateFailed') + ' balance snapshot';
    } finally {
      submitting = false;
    }
  }
...
<Modal {open} onclose={onclose} title={t('modals.editBalance')} size="md">
  <div class="form">
    {#if error}
