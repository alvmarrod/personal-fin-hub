...
describe('profile store', () => {
  ...
  it('activateProfile sets active, persists id, and notifies the api client', () => {
    ...
    expect(store.activeProfile()).toEqual({ id: 7, name: 'Gamma', default_fiscal_rule: null, timezone: 'Asia/Tokyo' });
  ...
  it('initProfiles restores a stored active profile', async () => {
    ...
    expect(store.hasActiveProfile()).toBe(true);
    expect(store.activeProfile()).toEqual({ id: 2, name: 'Beta', default_fiscal_rule: null, timezone: 'Asia/Tokyo' });
  ...
  it('renameProfile refreshes and updates the active profile name', async () => {
    ...
    expect(store.activeProfile()).toEqual({ id: 1, name: 'AlphaRenamed', default_fiscal_rule: null, timezone: 'Asia/Tokyo' });
  });
  ...
  it('deleteProfile keeps the session when deleting another profile', async () => {
    ...
    expect(store.hasActiveProfile()).toBe(true);
    expect(store.activeProfile()).toEqual({ id: 1, name: 'Alpha', default_fiscal_rule: null, timezone: 'Asia/Tokyo' });
  });
});
