...
- [x] **Fiscal-Rules P&L Engine — Phases 1–3**: true FIFO lots with `buy_date` (`services/pnl_rules.py` `compute_fifo`, shared by `get_realized_gains` and `_compute_fifo_cost_basis`); `PnlRule` registry (`spain`, `japan`, `default` copy of spain, `latest` legacy, `none`); proceeds currency (`payment_currency`/`fx_rate`); buy-date invested-historic conversion; closest-in-time rate fallback + `rate_fallbacks` flags; locale-inferred default rule. Phase 2 adds `fiscal_periods` (profile-scoped rule-per-date-range, overlap-rejected, CRUD `/fiscal-periods` + Settings "Fiscal Rules" section) and `transactions.fiscal_rule` snapshot frozen at sell creation (period edits never change past ops; timestamp edits re-resolve). Phase 3 adds the Tax page: ruleset-configurable fiscal-year start + dividend taxation, `GET /analytics/taxable-pnl` grouping realized gains + dividends per fiscal year with `fiscal_exemptions` applied (rate % exempt, optional fixed allowance, optional cap), `/tax` frontend page. Backend 0.17.0 + frontend 0.15.0; backend suite 1066, frontend 114, ruff + mypy + svelte-check + build + validate-i18n clean.
