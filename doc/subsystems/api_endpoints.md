...
- `GET /analytics/cash-flow?group_by=&start_date=&end_date=&display_currency=` — Cash flow analysis. Groups by period + type + currency + category. Each `CashFlowLine` now carries `category` (income_category for INCOME, investment_transaction_category for INVESTMENT_BUY/INVESTMENT_SELL, NULL for MONEY_OUT and excluded types). The frontend renders expandable rows: Inflows → INCOME (with subcategories) + INVESTMENT_SELL (with subcategories); Outflows → MONEY_OUT (flat) + INVESTMENT_BUY (with subcategories). Category-level breakdown is expanded on demand.
- `GET /analytics/cash-flow/transactions?period=&type=&category=&currency=&start_date=&end_date=` — Individual transactions for a specific cash-flow row. Returns a lightweight list of transactions matching the given period, type, category, and currency. Used by the frontend's lazy-loaded level-3 drill-down (expandable transaction list within each type row).
