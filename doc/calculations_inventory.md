...
| MetricCard | Active Sources | Not defined | ✅ | Count of unique entity-currency-category combinations from projected income. A simple count, not a financial calculation. |
| StackedBarChart | Income by Type | Not defined | ✅ | Grouped by income category (Salary, Other, Dividends, Interest, Cashback). Category resolved server-side: explicit `income_category` on the transaction/schedule wins; legacy rows fall back to income into an `EMPLOYER` entity → Salary, else Other. Two datasets per category: realized (from `incomeBySource` API, grouped by month) and projected (from `projectedIncome` API). All values converted to display currency. |
| Table | Income Sources | Not defined | ✅ | Per source-currency-category: category badge, realized (this month), projected (this month), total (all periods), schedule description/periodicity, next payment date. **Displays in native currency** (no conversion). |
